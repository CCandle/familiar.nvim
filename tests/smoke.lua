...
assert(config.brain.local_model.model_path == nil)
assert(models.configured_path(config.brain) == models.path())
assert(string.find(models.path(), vim.fs.joinpath("familiar", "models"), 1, true))
...
familiar.brain({ enabled = true, provider = "local_llama" })
